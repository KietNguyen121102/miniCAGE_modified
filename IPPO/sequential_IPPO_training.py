"""
Sequential IPPO training where Agent 1 observes intermediate state after Agent 0's action.
Agent 1 truly acts based on the observation that results from Agent 0's action.
"""
...
from sequential_IPPO_wrapper import IPPOSequentialObsEnv
...
USE_WANDB: bool = True
WANDB_PROJECT: str = "mini-cage-sequential-ippo"
GROUP_NAME: str = f"Sequential_IPPO_2Blue_{TOTAL_TIMESTEPS}"
...
def train_sequential_ippo():
    """Main Sequential IPPO training loop."""
    ...
    if USE_WANDB:
        ...
        run = wandb.init(
            ...
            group=GROUP_NAME,
            name=f"sequential_ippo_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            config={
                ...
                "agent_1_observes_after_agent_0": True,
            },
    ...
    print(f"Sequential observation: Agent 1 observes state after Agent 0's action")
    ...
    # Initialize environments - agent 0 gets initial obs
    current_obs_agent_0 = np.array([env.reset() for env in envs])
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(N_STEPS):
            ...
            # ====== Agent 0 selects action based on current state ======
            obs_tensor_0 = torch.FloatTensor(current_obs_agent_0).to(device)
            
            with torch.no_grad():
                action_0, log_prob_0, _, value_0 = agents[0].get_action_and_value(obs_tensor_0)
            
            actions_0 = action_0.cpu().numpy()
            log_probs_0 = log_prob_0.cpu().numpy()
            values_0 = value_0.cpu().numpy().flatten()
            
            # ====== Execute Agent 0 and get intermediate observations ======
            intermediate_obs_list = []
            rewards_0_list = []
            dones_partial_list = []
            
            for env_idx in range(N_ENVS):
                intermediate_obs, reward_0, done_partial, info_partial = envs[env_idx].step_agent_0(actions_0[env_idx])
                intermediate_obs_list.append(intermediate_obs)
                rewards_0_list.append(reward_0)
                dones_partial_list.append(done_partial)
            
            intermediate_obs_agent_1 = np.array(intermediate_obs_list)
            rewards_agent_0 = np.array(rewards_0_list)
            
            # ====== Agent 1 selects action based on INTERMEDIATE state ======
            obs_tensor_1 = torch.FloatTensor(intermediate_obs_agent_1).to(device)
            
            with torch.no_grad():
                action_1, log_prob_1, _, value_1 = agents[1].get_action_and_value(obs_tensor_1)
            
            actions_1 = action_1.cpu().numpy()
            log_probs_1 = log_prob_1.cpu().numpy()
            values_1 = value_1.cpu().numpy().flatten()
            
            # ====== Execute Agent 1 and get final state ======
            next_obs_list = []
            rewards_list = []
            dones_list = []
            
            for env_idx in range(N_ENVS):
                final_obs, rewards, dones, infos = envs[env_idx].step_agent_1(actions_1[env_idx])
                
                next_obs_list.append(final_obs)
                ...
                if dones[0]:  # Episode done
                    ...
                    # Reset environment and get new initial observation for agent 0
                    reset_obs = envs[env_idx].reset()
                    next_obs_list[-1] = [reset_obs, reset_obs]  # Both agents get same reset obs
            
            # Extract final observations and rewards
            next_obs_agent_0 = np.array([next_obs_list[env_idx][0] for env_idx in range(N_ENVS)])
            rewards_agent_1 = np.array([rewards_list[env_idx][1] for env_idx in range(N_ENVS)])
            dones = np.array([dones_list[env_idx][0] for env_idx in range(N_ENVS)])
            
            # Store in buffers
            # Agent 0: observed current state, acted, got reward_0
            buffers[0].add(
                current_obs_agent_0,
                actions_0,
                rewards_agent_0,
            ...
            # Agent 1: observed intermediate state, acted, got reward_1
            buffers[1].add(
                intermediate_obs_agent_1,
                actions_1,
                rewards_agent_1,
                dones,
                values_1,
            ...
            current_obs_agent_0 = next_obs_agent_0
        
        # Compute returns and advantages for each agent
        returns_and_advantages = []
        
        # Agent 0: bootstrap from final state
        obs_tensor_0 = torch.FloatTensor(current_obs_agent_0).to(device)
        with torch.no_grad():
            _, _, _, last_value_0 = agents[0].get_action_and_value(obs_tensor_0)
        last_values_0 = last_value_0.cpu().numpy().flatten()
        returns_0, advantages_0 = buffers[0].compute_returns_and_advantages(last_values_0, dones)
        returns_and_advantages.append((returns_0, advantages_0))
        
        # Agent 1: also bootstrap from final state
        obs_tensor_1 = torch.FloatTensor(current_obs_agent_0).to(device)  # Same final state
        with torch.no_grad():
            _, _, _, last_value_1 = agents[1].get_action_and_value(obs_tensor_1)
        last_values_1 = last_value_1.cpu().numpy().flatten()
        returns_1, advantages_1 = buffers[1].compute_returns_and_advantages(last_values_1, dones)
        returns_and_advantages.append((returns_1, advantages_1))
        ...
        if update % 1 == 0:  # Log every update
            ...
            if len(episode_rewards[0]) > 0:
                ...
                # Team performance
                team_rewards = [(recent_rewards_0[i] + recent_rewards_1[i]) for i in range(len(recent_rewards_0))]
                log_dict["rollout/team_reward_mean"] = np.mean(team_rewards)
                log_dict["rollout/team_reward_std"] = np.std(team_rewards)
                
                # Print summary (SB3-style)
                mean_reward = np.mean(recent_rewards_0)
                mean_length = np.mean(recent_lengths)
                team_mean = np.mean(team_rewards)
                print(f"------------------------------------------")
                print(f"| rollout/                |              |")
                print(f"|    ep_len_mean          | {mean_length:<12.1f} |")
                print(f"|    ep_rew_mean (agent0) | {mean_reward:<12.2f} |")
                print(f"|    team_reward_mean     | {team_mean:<12.2f} |")
                print(f"| time/                   |              |")
                print(f"|    fps                  | {fps:<12.0f} |")
                print(f"|    total_timesteps      | {global_step:<12} |")
                print(f"|    update               | {update:<12} |")
                print(f"------------------------------------------")
    ...
    if USE_WANDB:
        # Log final artifacts
        for agent_idx in range(num_agents):
            artifact = wandb.Artifact(f"agent_{agent_idx}_final", type="model")
            artifact.add_file(str(SAVE_DIR / f"agent_{agent_idx}_final.pt"))
            run.log_artifact(artifact)
        wandb.finish()


if __name__ == "__main__":
    train_sequential_ippo()